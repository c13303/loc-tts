 ...
 */
import { updatePlaybackSpeed, setupSpeedEventHandlers } from './effects-speed.js';

/**
 * Set up all event handlers
 * @param {Object} processor - The AudioProcessor instance
 */
export function setupEventHandlers(processor) {
...
function setupEffectControlHandlers(processor) {
    ...
    $('.effect-slider').on('input', (e) => {
        ...
        if (processor.effectsMode === 'realtime') {
            processor.updateEffectParameters();
        }
    });
...
function setupButtonHandlers(processor) {
    // Synthesize button
    $('#synthesize').on('click', () => processor.synthesizeAudio());

    // Save preset
    $('#save-preset').on('click', () => processor.showPresetModal());

    // Refresh presets
    $('#refresh-presets').on('click', () => {
        console.log("Refreshing presets...");
        processor.loadPresetsFromServer();
    });
}
